...
#' @export
bkt.bforce.search.grid <- function(step.size = 0.1, 
  ...
  data.table (
    init = rep(all.init, times = (n.learn * n.guess * n.slip)),
    learn = rep(
      all.learn,
      each = n.init,
      times = (n.guess * n.slip)
    ),
    guess = rep(
      all.guess,
      each = (n.init * n.learn),
    ...
    slip = rep(all.slip, each = (n.init * n.learn *
      n.guess))
  )
}
...
#' }
#' @export
bkt.bforce.search <- function(opps, 
                              search.grid = bkt.bforce.search.grid(), 
                              fit.metric = bkt.rmse, 
                              minimize = T) {
  ...
  for (i in 1:nrow(search.grid)) {
    #bf$fits[i] = fit.metric(opps, search.grid[i, ])
    # set(search.grid, i, 5L, fit.metric(opps, data.frame(init = 1L, learn = 2L, guess = 3L, slip = 4L)))
    search.grid$time[i] = system.time(search.grid$rmse[i] <- fit.metric(opps, data.frame(init = 1L, learn = 2L, guess = 3L, slip = 4L)))
  }
  # bf$fits = search.grid$fits
  #bf$fits = search.grid[, fit.metric(opps, as.bkt.params(list(init = init, learn = learn, guess = guess, slip = slip)))]
