                                ...
                                max.slip=(0.5 - step.size) )
{
  ...
  data.table( init=rep(all.init, times=(n.learn*n.guess*n.slip) ),
...
#' }
#' @export
bkt.bforce.search <- function( opps, search.grid=bkt.bforce.search.grid(), fit.metric=bkt.rmse, minimize=T )
{
  bf = list(search.grid=search.grid, fit.metric=fit.metric)
  bf$cost = system.time(bf$fits <- apply(search.grid, 1, function(p){fit.metric( opps, p )}))
